...
def create():
    ...
    def show_entry_fields():
        print("Latitude: %s\nLongitude: %s\nMinimum Frequency: %s\nMaximum Frequency: %s" % (lat.get(), long.get(), min_freq.get(), max_freq.get()))
    def exit():
        window.destroy()

    ttk.Button(win, text = "Exit", command = exit).grid(row=5, column=1, sticky=tk.W, pady=4)
    ttk.Button(win, text='Enter', command=show_entry_fields).grid(row=4, column=1, sticky=tk.W, pady=4)
